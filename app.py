...
# دالة توليد الأسئلة المحدثة بحظر التداخل والتفرقة الدقيقة بين الأطفال والكبار
def generate_questions_via_gemini(category, topic, count, lang="ar", age=None):
    try:
        model = genai.GenerativeModel('gemini-2.5-flash', generation_config={"response_mime_type": "application/json"})
        
        # ضبط فئة العمر والأسلوب
        if age:
            audience_instruction = f"""
            المستهدف: طفل عمره بالضبط ({age}) سنوات.
            - يجب أن تكون الأسئلة قصيرة جداً، سهلة للغاية، ومناسبة لعقل طفل بعمر {age} سنوات.
            - استخدم كلمات مشجعة ومفهومة للأطفال بدون أي مصطلحات معقدة.
            """
        else:
            audience_instruction = """
            المستهدف: الكبار والشباب.
            - أسئلة ثقافية ممتعة، متوسطة إلى متقدمة الصعوبة، تناسب عقول الكبار.
            """

        # ضبط شروط التخصص الصارمة
        topic_instruction = f"المجال المطلوب حصراً: ({topic})."
        if topic == "إسلاميات":
            topic_instruction += " تمبيه صارم جداً: يجب أن تكون كافة الأسئلة متعلقة حصراً بالدين الإسلامي، القرآن الكريم، السيرة النبوية، العبادات، أو أركان الإسلام. يمنع منعاً باتاً ذكر أي سؤال عن الكواكب أو العلوم أو أي مجال خارجي."
        elif topic == "لغة عربية":
            topic_instruction += " تمبيه صارم: يجب أن تكون الأسئلة عن القواعد، المفردات، أو الأدب العربي فقط."
        elif topic == "علوم":
            topic_instruction += " تمبيه صارم: يجب أن تكون الأسئلة في مجالات الأحياء، الفيزياء، الكيمياء، أو الكواكب والعلوم الطبيعية."

        prompt = f"""
        أنت مصمم مسابقات تعليمية محترف جداً للمملكة العربية السعودية.
        أنشئ بالضبط ({count}) أسئلة متعددة الخيارات.

        {audience_instruction}
        {topic_instruction}

        الشروط الواجب تطبيقها:
        1. "السؤال": مباشر وواضح ومفهوم.
        2. "الخيار 1 - الصحيح": الإجابة الصحيحة الأكيدة 100%.
        3. "الخيار 2", "الخيار 3", "الخيار 4": خيارات خاطئة واضحة ذات صلة بنفس المجال.
        4. عدم تكرار الأسئلة أو خلط المجالات إطلاقاً.
    ...
    # الاحتياطي المنقح حسب المجال
    if topic == "إسلاميات":
        if age and age <= 10:
            fallback = [
                {"السؤال": "ما هو الدين الذي نعبده بحب؟", "الخيار 1 - الصحيح": "الإسلام", "الخيار 2": "الصدق", "الخيار 3": "الخير", "الخيار 4": "النور"},
                ...
                {"السؤال": "ما هو الكتاب الذي أنزله الله على نبينا محمد؟", "الخيار 1 - الصحيح": "القرآن الكريم", "الخيار 2": "التوراة", "الخيار 3": "الإنجيل", "الخيار 4": "الزبور"},
                {"السؤال": "ما هي أول صلاة نصلّيها في اليوم؟", "الخيار 1 - الصحيح": "صلاة الفجر", "الخيار 2": "صلاة الظهر", "الخيار 3": "صلاة العصر", "الخيار 4": "صلاة العشاء"}
            ]
        else:
            fallback = [
                {"السؤال": "كم عدد سور القرآن الكريم؟", "الخيار 1 - الصحيح": "114 سورة", "الخيار 2": "100 سورة", "الخيار 3": "120 سورة", "الخيار 4": "110 سور"},
                {"السؤال": "ما هي أطول سورة في القرآن الكريم؟", "الخيار 1 - الصحيح": "سورة البقرة", "الخيار 2": "سورة آل عمران", "الخيار 3": "سورة النساء", "الخيار 4": "سورة المائدة"},
                {"السؤال": "في أي مدينة ولد النبي محمد صلى الله عليه وسلم؟", "الخيار 1 - الصحيح": "مكة المكرمة", "الخيار 2": "المدينة المنورة", "الخيار 3": "الطائف", "الخيار 4": "الرياض"},
                {"السؤال": "ما هو أول الأركان الخمسة للإسلام؟", "الخيار 1 - الصحيح": "الشهادتان", "الخيار 2": "إقام الصلاة", "الخيار 3": "إيتاء الزكاة", "الخيار 4": "صوم رمضان"},
                {"السؤال": "ما اسم أول خلفاء الراشدين؟", "الخيار 1 - الصحيح": "أبو بكر الصديق", "الخيار 2": "عمر بن الخطاب", "الخيار 3": "عثمان بن عفان", "الخيار 4": "علي بن أبي طالب"}
            ]
    else:
        fallback = [
            {"السؤال": "ما هي عاصمة المملكة العربية السعودية؟", "الخيار 1 - الصحيح": "الرياض", "الخيار 2": "جدة", "الخيار 3": "الدمام", "الخيار 4": "مكة المكرمة"},
            {"السؤال": "ما هو الطائر الذي يعتبر رمزاً للصقر والشجاعة؟", "الخيار 1 - الصحيح": "الصقر", "الخيار 2": "العصفور", "الخيار 3": "الحمامة", "الخيار 4": "البومة"},
            {"السؤال": "كم عدد ألوان علم المملكة العربية السعودية؟", "الخيار 1 - الصحيح": "لونان (الأخضر والأبيض)", "الخيار 2": "ثلاثة ألوان", "الخيار 3": "أربعة ألوان", "الخيار 4": "لون واحد"},
            {"السؤال": "ما هو الحيوان السريع المعروف بسفينة الصحراء؟", "الخيار 1 - الصحيح": "الجمل", "الخيار 2": "الخيل", "الخيار 3": "الغزال", "الخيار 4": "الأسد"}
        ]
    ...
    if 'my_live_room' not in st.session_state:
        ...
        if mode_select == "توليد تلقائي ذكي عبر جيمني AI":
            ...
            if st.button("🎲 إنشاء غرفتك عبر الذكاء الاصطناعي", use_container_width=True):
                with st.spinner("جاري صياغة أسئلة عربية متخصصة ودقيقة عبر جيمني... ⚡"):
...
# صفحة اختبر نفسك الفردية (ثقف نفسك)
    ...
    if 'solo_questions' not in st.session_state:
        ...
        if st.button("🎯 ابدأ إطلاق وتوليد المسابقة فوراً", use_container_width=True):
            with st.spinner("جاري صياغة أسئلة مخصصة بدقة ومناسبة جداً للفئة المختارة... 🔥"):
